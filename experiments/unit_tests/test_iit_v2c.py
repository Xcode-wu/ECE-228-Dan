...
# Train annotation
annotations_train = load.load_annotations(dataset_path, annotation_file_train)
load.summary(annotations_train)

clips_name_train, captions_train = load.clipsname_captions(annotations_train)
print(clips_name_train[0], captions_train[0])
print(len(clips_name_train), len(captions_train))
print()

# Test annotation
annotations_test = load.load_annotations(dataset_path, annotation_file_test)
load.summary(annotations_test)

clips_name_test, captions_test = load.clipsname_captions(annotations_test)
...
config.MODE = 'test'
annotation_file = config.MODE + '.txt'
clips_test, targets_test, vocab_test, config = load.parse_dataset(config, annotation_file, vocab=vocab)
print(vocab == vocab_test)
test_dataset = load.FeatureDataset(clips_test, targets_test)
